...
"""
Prim's Algorithm
The function prim performs Prim's algorithm for finding MST.
...
def prim(adjList, adjMat):
    ...
    # Pick an start vertex and set cost to 0
    start = adjList[0]
    start.cost = 0
    ...
    while not Q.isEmpty():
        ...
        for neighbor in v.neigh:
            ...
            if not neighbor.visited:
                edge = Edge(v, neighbor, adjMat[v.rank][neighbor.rank])
                if neighbor.cost > edge.weight:
...
The function prim performs Kruskal's algorithm for finding MST.
...
"""
The function tsp utilizes the combination of MST and DFS to find the 
optimal solution for a variation of the Traveling Salesman Problem.
...
OUTPUTS
tour: An array which indicates the tour of vertex ranks

"""
def tsp(adjList, start):
    ##### Your implementation goes here. #####
    # First iterate through the list of vertices as flag them all as unvisited
    for vertice in adjList:
    ...
    while stack:
        ...
        # build the tour list by appending the rank of each new vertex visited 
        if current.rank not in tour:
        ...
        # we are only interested in the edges of the MST
